...
( function(root) {
    ...
    Parse._ajax = function(method, url, data, success, error) {
      ...
      if (Parse._isNode) {
        ...
        xhr.setRequestHeader("User-Agent", "Parse/" + Parse.VERSION + " (NodeJS " + process.versions.node + ")");
      }
      xhr.send(data);
    ...
    Parse._request = function(options) {
      ...
      if (route !== "batch" && route !== "classes" && route !== "events" && route !== "files" && route !== "functions" && route !== "login" && route !== "push" && route !== "requestPasswordReset" && route !== "rest_verify_analytics" && route !== "users" && route !== "jobs") {
      ...
      return Parse._ajax(method, url, data).then(null, function(response) {
        ...
        if (response && response.responseText) {
          ...
          error = new Parse.Error(Parse.Error.CONNECTION_FAILED, "XMLHttpRequest failed: " + JSON.stringify(response));
        }
        ...
        var pointerToId = function(object) {
          if ( object instanceof Parse.Object) {
            ...
            if (self._targetClassName !== object.className) {
              throw "Tried to create a Parse.Relation with 2 different types: " + self._targetClassName + " and " + object.className + ".";
            }
            return object.id;
          }
          return object;
...
( function(root) {
    ...
    var readAsync = function(file, type) {
      ...
      if ( typeof (FileReader) === "undefined") {
        return Parse.Promise.error(new Parse.Error(Parse.Error.FILE_READ_ERROR, "Attempted to use a FileReader on an unsupported browser."));
      }
    ...
    Parse.File = function(name, data, type) {
      ...
      if (_.isArray(data)) {
        ...
        if (matches && matches.length > 0) {
          ...
          this._source = Parse.Promise.as((matches.length === 4 ? matches[3] : matches[2]), matches[1]);
        } else {
...
( function(root) {
    ...
    Parse.Object = function(attributes, options) {
      ...
      this._hashedJSON = {};
      // Hash of values of containers at last save.
        ...
        Parse._objectEach(this.attributes, function(value, key) {
          if ( value instanceof Parse.Object) {
            ...
            if (self._resetCacheForKey(key)) {
              self.set(key, new Parse.Op.Set(value), {
                silent : true
              });
            }
          }
    ...
    Parse.Object._deepSaveAsync = function(object, options) {
      ...
      return promise.then(function() {
        return Parse.Promise._continueWhile(function() {
          ...
          if (batch.length === 0) {
            return Parse.Promise.error(new Parse.Error(Parse.Error.OTHER_CAUSE, "Tried to save a batch with a cycle."));
          }
          ...
          return readyToStart._continueWith(function() {
            return Parse._request({
              ...
              if (error) {
                return Parse.Promise.error(new Parse.Error(error.code, error.error));
              }
      ...
      validate : function(attrs, options) {
        if ("name" in attrs && attrs.name !== this.getName()) {
          ...
          if (this.id && this.id !== attrs.objectId) {
            ...
            return new Parse.Error(Parse.Error.OTHER_CAUSE, "A role's name can only be set before it has been saved.");
          }
          if (!_.isString(newName)) {
            return new Parse.Error(Parse.Error.OTHER_CAUSE, "A role's name must be a String.");
          }
          if (!(/^[0-9a-zA-Z\-_ ]+$/).test(newName)) {
            return new Parse.Error(Parse.Error.OTHER_CAUSE, "A role's name can only contain alphanumeric characters, _," + " -, and spaces.");
          }
        }
...
( function(root) {
    ...
    }, /** @lends Parse.User */
    {
      ...
      logOut : function() {
        ...
        Parse.localStorage.removeItem(Parse._getParsePath(Parse.User._CURRENT_USER_KEY));
      },
      ...
      current : function() {
        ...
        var userData = Parse.localStorage.getItem(Parse._getParsePath(Parse.User._CURRENT_USER_KEY));
        if (!userData) {
      ...
      _saveCurrentUser : function(user) {
        ...
        Parse.localStorage.setItem(Parse._getParsePath(Parse.User._CURRENT_USER_KEY), JSON.stringify(json));
      },
...
( function(root) {
    ...
    Parse.Query = function(objectClass) {
      ...
      this._include = [];
      this._limit = -1;
      // negative limit means, do not send a limit
      this._skip = 0;
      this._extraOptions = {};
    };
